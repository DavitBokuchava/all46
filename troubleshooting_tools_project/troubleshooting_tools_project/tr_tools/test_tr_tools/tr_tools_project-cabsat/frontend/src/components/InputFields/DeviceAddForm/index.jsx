...
const DeviceAddForm = ({ onClickSubmit, disable }) => {
    ...
    const addDeviceItemError = useSelector(state => state.devices.addDevice.error)
    React.useEffect(() => {
    ...
    const handleSubmit = (e) => {
        ...
        // console.log(addDeviceItemError)
    }
            ...
            <div className={st.formsWrapper}>
                <form onSubmit={handleSubmit}>
                    <label
                        htmlFor="deviceName"
                        className={cn({
                            [st.errorLabels]: addDeviceItemError && addDeviceItemError.deviceNameError
                        })}>
                        {addDeviceItemError && addDeviceItemError.deviceNameError ? addDeviceItemError.deviceNameError : "Device Name"}
                    </label>
                    <input
                        className={cn(st.input, {
                            [st.errorFields]: addDeviceItemError && addDeviceItemError.deviceNameError
                        })}
                        type="text"
                        id="deviceName"
                        name="deviceName"
                    ...
                    <label htmlFor="deviceIpAddress"
                        className={cn({
                            [st.errorLabels]: addDeviceItemError && addDeviceItemError.deviceIpAddressError
                        })}>I{addDeviceItemError && addDeviceItemError.deviceIpAddressError ? addDeviceItemError.deviceIpAddressError : "Ip Address"}</label>
                    <input
                        className={cn(st.input, {
                            [st.errorFields]: addDeviceItemError && addDeviceItemError.deviceIpAddressError
                        })}
                        type="text"
                        id="deviceIpaddress"
                    ...
                    {deviceFields.technology === 'gpon' && <label htmlFor="vendor">VENDOR</label>}

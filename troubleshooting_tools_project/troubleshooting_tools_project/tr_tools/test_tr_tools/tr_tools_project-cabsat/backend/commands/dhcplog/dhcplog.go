...
func DhcpLog(w http.ResponseWriter, r *http.Request) {
	...
	if commands.MacAddress == "" || commands.Vendor == "" || commands.IpAddress == "" || commands.Command == "" || commands.CustomerId == "" {
	...
	fmt.Println(commands.MacAddress, commands.Command, "#### check commands in dhcp")
	fmt.Println("external_scripts/showcommands/dhcplog/" + commands.Command)

	cmd := exec.Command("python3", "external_scripts/showcommands/dhcplog/"+commands.Command+".py", commands.MacAddress)

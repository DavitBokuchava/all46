...
#         print(command)
        ...
        result = tn.expect([b"\) ----", b"#", b"<K> "], command_timeout)
        output += result[2].decode('ascii')
        # Failure: Board type is invalid
        # Failure: Board type error
        if str(output).find("Failure: Board type error") >= 0:
            print("Failure: Board type error")
            quit_func(1)
            sys.exit()
        if str(output).find("Failure: Board type is invalid") >= 0:
            print("Failure: Board type is invalid")
            quit_func(1)
            sys.exit()

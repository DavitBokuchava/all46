...
try:
    with Telnet(olt_ip, 23, 5) as tn:
        ...
        command_2 = "display ont traffic " + port + " " + virt_port
        ...
        result = tn.expect([b"expect"], command_timeout)
        output += result[2].decode('ascii')
        
        up_traffic = output.split("Up traffic (kbps)          :  ")[1].split("Down traffic")[0]
        up_traffic = "Up traffic (kbps): " + up_traffic.strip()
        

        
        down_traffic = output.split("Down traffic (kbps)        :  ")[1].split("-----")[0]
        down_traffic = "Down traffic (kbps): " + down_traffic.strip()      

...
try:
    with Telnet(olt_ip, 23, 5) as tn:
        ...
        for i in str(output).split("\\r\\n"):
            i = i.replace("---- More ( Press 'Q' to break ) ----\\x1b[37D                                     \\x1b[37D","")
            i = i.split()
            
            if len(i) > 5 and len(str(i[3])) == 16 and i[3].find("0000") != 0:
                count_sn += 1
                print(i[0] + i[1] + " " + i[2] + "  SN: " + i[3])
        print()
        print("Total Serial Numbers: " + str(count_sn))

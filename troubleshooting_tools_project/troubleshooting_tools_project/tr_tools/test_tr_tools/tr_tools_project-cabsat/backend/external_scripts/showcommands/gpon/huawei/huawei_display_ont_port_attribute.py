...
try:
    with Telnet(olt_ip, 23, 5) as tn:
        ...
        result = tn.expect([b"#"], command_timeout)
        output += result[2].decode('ascii')
        if str(output).find("Failure: The ONT current management mode does not support this operation") >= 0:
            print("Failure: The ONT current management mode does not support this operation")
            quit_func(2)
            sys.exit()
        if str(output).find("% Parameter error, the error locates at '^'") >= 0:
